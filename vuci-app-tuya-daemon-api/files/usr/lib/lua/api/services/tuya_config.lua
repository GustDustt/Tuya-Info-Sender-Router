...
local ConfigTuya = Tuya:section(
	"example", -- UCI config name
	"example"  -- UCI section type
)
ConfigTuya:make_primary()
ConfigTuya.default_options.id.maxlength = 8 -- Default id option can also have validations
-- ConfigTuya.order_by = "option" -- Order UCI config by provided option
-- ConfigTuya.sort_response_by = "option" -- Order API response by provided option

function ConfigTuya:create_defaults(sid)
	...
	local opt_select = ConfigTuya:option("select")
		function opt_select:validate(value)
			return self.dt:check_array(value, { "first", "second" })
		end

	local opt_multi_select = ConfigTuya:option("multi_select", { list = true })
		opt_multi_select.maxlength = 64

	local opt_test = ConfigTuya:option("test")
